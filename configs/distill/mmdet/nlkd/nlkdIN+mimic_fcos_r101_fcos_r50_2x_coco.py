...
model = dict(
    ...
    distiller=dict(
        ...
        student_recorders=dict(fpn=dict(type='ModuleOutputs', source='neck'),
                               bbox_head=dict(type='ModuleOutputs', source='bbox_head.conv_reg')),
        ...
        distill_losses=dict(
            ...
            loss_bbox_head=dict(type='mmdet.SmoothL1Loss', loss_weight=10),
        ),  
        loss_forward_mappings=dict(
            ...
            loss_bbox_head=dict(
                pred=dict(from_student=True, recorder='bbox_head', data_idx=None),
                target=dict(from_student=False, recorder='bbox_head', data_idx=None))
        )
    )
)
