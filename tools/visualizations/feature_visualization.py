...
def parse_args():
    ...
    parser.add_argument(
        '--draw-bbox',
        action='store_true',
        help='only draw bounding boxes')  # 12.02
    parser.add_argument(
        '--cfg-options',
        nargs='+',
        action=DictAction,
...
def main(args):
    ...
    with recorder_manager:
        ...
        result = inference_detector(model, args.img) #10/14
        
        # 如果在instance detection中只想显示检测框，删除mask信息
        if args.draw_bbox and hasattr(result.pred_instances, 'masks'):
            del result.pred_instances.masks     # 12.02
