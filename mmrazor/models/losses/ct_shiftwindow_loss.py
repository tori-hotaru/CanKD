...
class DualFeatureWindowSelectorDynamic(nn.Module):
    def __init__(self, in_channels, top_k, embed_dim, num_heads, 
                 num_windows_h=2, num_windows_w=2):
        """
        Args:
            ...
            num_windows_h (int): 高度方向窗口的数量（动态决定窗口大小）
            num_windows_w (int): 宽度方向窗口的数量（动态决定窗口大小）
        """
        super(DualFeatureWindowSelectorDynamic, self).__init__()
        self.top_k = top_k
        ...
        # 嵌入层，将教师特征图的窗口展平后映射到嵌入空间
        # 这里需要考虑窗口大小是动态的，因此使用卷积层代替线性层
        # 通过使用一个共享的卷积核处理所有窗口
        ...
        # 可选：位置编码（如果需要捕捉位置信息）
        self.position_embedding = nn.Parameter(torch.randn(1, 1, embed_dim))

    def forward(self, y_s, y_t):
        """
        Args:
            y_s (torch.Tensor): 学生特征图，形状为 (batch_size, channels, height, width)
            y_t (torch.Tensor): 教师特征图，形状为 (batch_size, channels, height, width)
        
        Returns:
            selected_windows_s (torch.Tensor): 选中的学生窗口，形状为 (batch_size, top_k, channels, window_size_h, window_size_w)
            selected_windows_t (torch.Tensor): 选中的教师窗口，形状为 (batch_size, top_k, channels, window_size_h, window_size_w)
            attention_scores (torch.Tensor): 选中窗口的注意力分数，形状为 (batch_size, top_k)
        """
        batch_size, channels, height, width = y_s.size()

        # 动态计算窗口大小和步幅
        window_size_h = math.ceil(height / self.num_windows_h)
        window_size_w = math.ceil(width / self.num_windows_w)
        stride_h = window_size_h
        stride_w = window_size_w

        # 使用 unfold 提取所有窗口
        windows_t = F.unfold(y_t, kernel_size=(window_size_h, window_size_w), 
                             stride=(stride_h, stride_w))  # (batch_size, channels * window_size_h * window_size_w, num_windows)
        num_windows = windows_t.size(-1)

        # 重新排列为 (batch_size, num_windows, channels, window_size_h, window_size_w)
        windows_t = windows_t.transpose(1, 2).contiguous().view(
            batch_size, num_windows, channels, window_size_h, window_size_w
        )

        # 对学生特征图进行相同的窗口提取
        windows_s = F.unfold(y_s, kernel_size=(window_size_h, window_size_w), 
                             stride=(stride_h, stride_w))  # (batch_size, channels * window_size_h * window_size_w, num_windows)
        windows_s = windows_s.transpose(1, 2).contiguous().view(
            batch_size, num_windows, channels, window_size_h, window_size_w
        )

        # 生成嵌入
        # 首先，将窗口展平为 (batch_size, num_windows, channels, window_size_h, window_size_w)
        # 然后，通过卷积层处理每个窗口
        # 需要将窗口视为小的特征图，进行逐窗口的嵌入
        # 重塑为 (batch_size * num_windows, channels, window_size_h, window_size_w)
        windows_t_reshaped = windows_t.view(batch_size * num_windows, channels, window_size_h, window_size_w)
        embeddings = self.embedding(windows_t_reshaped)  # (batch_size * num_windows, embed_dim, window_size_h, window_size_w)

        # 全局平均池化以得到每个窗口的嵌入向量
        embeddings = F.adaptive_avg_pool2d(embeddings, (1, 1)).view(batch_size, num_windows, self.embed_dim)  # (batch_size, num_windows, embed_dim)

        # 添加位置编码（可选）
        embeddings = embeddings + self.position_embedding  # 广播到 (batch_size, num_windows, embed_dim)

        # 计算自注意力
        attn_output, attn_weights = self.self_attn(embeddings, embeddings, embeddings)  # attn_weights: (batch_size, num_windows, num_windows)

        # 计算每个窗口的全局注意力得分
        # 这里使用注意力权重的平均值作为得分
        attention_scores = attn_weights.mean(dim=1)  # (batch_size, num_windows)

        # 选择 top_k 个窗口
        topk_scores, topk_indices = torch.topk(attention_scores, self.top_k, dim=1)  # (batch_size, top_k)

        # 使用 gather 从 windows_s 和 windows_t 中选择对应的窗口
        # 首先调整 topk_indices 的形状以便进行 gather
        # topk_indices: (batch_size, top_k) -> (batch_size, top_k, 1, 1, 1)
        topk_indices_expanded = topk_indices.view(batch_size, self.top_k, 1, 1, 1).expand(
            -1, -1, channels, window_size_h, window_size_w
        )  # (batch_size, top_k, channels, window_size_h, window_size_w)

        # 将 windows_s 和 windows_t 的形状调整为 (batch_size, num_windows, channels, window_size_h, window_size_w)
        # 使用 gather 需要将 dimension=1，对应于 num_windows
        selected_windows_s = torch.gather(windows_s, 1, topk_indices_expanded)  # (batch_size, top_k, channels, window_size_h, window_size_w)
        selected_windows_t = torch.gather(windows_t, 1, topk_indices_expanded)  # (batch_size, top_k, channels, window_size_h, window_size_w)

        return selected_windows_s, selected_windows_t, topk_scores
...
#     x1 = torch.randn(4,256,273,273)
#     x2 = torch.rand(4,256,273,273)
...
if __name__ == "__main__":
    # 假设输入特征图为 (batch_size=2, channels=3, height=10, width=14)
    batch_size = 2
    channels = 3
    height = 10
    width = 14
    y_s = torch.randn(batch_size, channels, height, width)
    y_t = torch.randn(batch_size, channels, height, width)

    # 定义选择 top 3 窗口，嵌入维度为 64，注意力头数为 4
    top_k = 3
    embed_dim = 64
    num_heads = 4
    # 设定窗口数量为高度方向2个，宽度方向3个
    num_windows_h = 2
    num_windows_w = 3

    selector = DualFeatureWindowSelectorDynamic(
    ...
    selected_windows_s, selected_windows_t, attention_scores = selector(y_s, y_t)

    print("Selected windows from student shape:", selected_windows_s.shape)
    print("Selected windows from teacher shape:", selected_windows_t.shape)
