...
class NonLocalAttention(nn.Module):
    ...
    def forward(self, y_s, y_t):
        ...
        y_s_ = torch.matmul(f_div_C_student, g_y_s)
        ...
        y_t_ = torch.matmul(f_div_C_teacher, g_y_t)
...
class AttProLoss(nn.Module):
    def __init__(self, 
                 ...
                 tau=1.0, 
                 loss_weight=1.0):
        super(AttProLoss, self).__init__()
        self.in_channels = in_channels
...
@MODELS.register_module()
class AttPro_L2_Loss(nn.Module):
    def __init__(self, 
                 in_channels, 
                 inter_channels=None, 
                 dimension=2, 
                 ...
                 tau=1.0, 
                 loss_weight=1.0):
        super(AttPro_L2_Loss, self).__init__()
        self.in_channels = in_channels
        self.inter_channels = inter_channels if inter_channels else in_channels // 2
        if self.inter_channels == 0: self.inter_channels = 1
        self.tau = tau
...
if __name__ == '__main__':
    ...
    att_loss = AttProLoss(in_channels=32, dimension=2)
    loss = att_loss(y_s, y_t)
    print(loss)

    att_l2_loss = AttPro_L2_Loss(in_channels=32, dimension=2)
