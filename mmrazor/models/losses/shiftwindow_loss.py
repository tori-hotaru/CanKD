...
class PatchAttentionMask(nn.Module):
    ...
    def forward(self, x):
        ...
        # 选择得分最高的 k 个补丁
        topk_scores, topk_indices = torch.topk(scores, self.k, dim=1)  # topk_indices: (batch_size, k)

        # 创建一个 one-hot 编码的选中补丁
        one_hot = torch.zeros(batch_size, L, device=x.device)
        one_hot.scatter_(1, topk_indices, 1)  # 形状: (batch_size, L)

        # 将 one-hot 编码扩展并重复以匹配 patch_size * patch_size
        one_hot = one_hot.unsqueeze(1)  # (batch_size, 1, L)
        # Change: Repeat for channels * patch_size * patch_size instead of just patch_size * patch_size
        one_hot = one_hot.expand(-1, channels * self.patch_size * self.patch_size, -1)

        # 使用 Fold 将选中的补丁映射回空间掩码
        # Change: Use the same stride as unfold
        mask = F.fold(one_hot, output_size=(height, width), 
                     kernel_size=self.patch_size, 
                     stride=self.stride)  # Changed stride from 1 to self.stride
...
class ShiftWindowLoss(nn.Module):
    def __init__(self, patch_sizes, k, channels_list, strides=None, alpha=0.7, beta=0.3):
        ...
        """
        super(ShiftWindowLoss, self).__init__()
...
if __name__ == "__main__":
    ...
    swl = ShiftWindowLoss(patch_sizes=[3,5,7], k=[50,50,50], channels_list=[128,128,128], strides=[1,1,1], alpha=0.7, beta=0.3)
    loss = swl(x, y)
    print("计算的loss:", loss)
