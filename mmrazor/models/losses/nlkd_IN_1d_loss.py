...
class NLKD_IN_1d_Loss(nn.Module):
    def __init__(self, 
                 in_channels, 
                 inter_channels=None, 
                 dimension=1, 
                 ...
                 tau=1.0, 
                 loss_weight=1.0):
        super(NLKD_IN_1d_Loss, self).__init__()
        self.in_channels = in_channels
    ...
    def forward(self, s_feature, t_feature):
        ...
        if s_feature.shape[1] > s_feature.shape[2]:
            s_feature = s_feature.transpose(1, 2)
            t_feature = t_feature.transpose(1, 2)
        if s_feature.shape[1] != t_feature.shape[1]:
            conv = nn.Conv1d(s_feature.shape[1], t_feature.shape[1], kernel_size=1, bias=False).to(s_feature.device)
            s_feature = conv(s_feature)
        if s_feature.shape[2] != t_feature.shape[2]:
            align = nn.Linear(s_feature.shape[2], t_feature.shape[2], bias=False).to(s_feature.device)
            s_feature = align(s_feature)
...
if __name__ == '__main__':
    y_s = torch.randn(2, 130, 167)
    y_t = torch.randn(2, 256, 200)

    nlkd_loss = NLKD_IN_1d_Loss(in_channels=256, dimension=1)
