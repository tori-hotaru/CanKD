...
class WaveletAttLoss(nn.Module):
    def __init__(self, 
                 in_channels, 
                 inter_channels=None, 
                 dimension=2, 
                 ...
                 tau=1.0, 
                 loss_weight=1.0):
        super(WaveletAttLoss, self).__init__()
        self.in_channels = in_channels
...
@MODELS.register_module()
class WaveletAtt_L2_Loss(nn.Module):
    def __init__(self, 
                 in_channels, 
                 inter_channels=None, 
                 dimension=2, 
                 ...
                 tau=1.0, 
                 loss_weight=1.0):
        super(WaveletAtt_L2_Loss, self).__init__()
        self.in_channels = in_channels
        self.inter_channels = inter_channels if inter_channels else in_channels // 2
        if self.inter_channels == 0: self.inter_channels = 1
        self.tau = tau
...
if __name__ == '__main__':
    ...
    wavelet_att_loss = WaveletAttLoss(in_channels=32, dimension=2)
    loss = wavelet_att_loss(y_s, y_t)
    print(loss)

    wavelet_att_l2_loss = WaveletAtt_L2_Loss(in_channels=32, dimension=2)
    loss = wavelet_att_l2_loss(y_s, y_t)
    print(loss)
