...
if __name__ == '__main__':
    ...
    # 读取并预处理图像
    im = cv2.imread("New_project/kodim23.png")
    if im is None:
        raise ValueError("图像读取失败，请检查路径和文件名。")
    
    from PIL import Image

    im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)  # 转换为RGB
    original_img = Image.fromarray(im)
    original_img.save("original.png")

    im = np.expand_dims(im, axis=0)
    print(f"扩展维度后的形状: {im.shape}")  # (1, H, W, C)

    im = np.concatenate([im]*5, axis=0)
    print(f"批次复制后的形状: {im.shape}")  # (5, H, W, C)

    im = np.transpose(im, [0, 3, 1, 2])
    print(f"转置后的形状: {im.shape}")  # (5, C, H, W)
    ...
    # 检查归一化
    print(f"归一化后的最小值: {im.min()}, 最大值: {im.max()}")

    im = torch.from_numpy(im).to(device)

    # SpaceToDepth 和 DepthToSpace 操作
    test = SpaceToDepth(w_factor=1)(im)
    test = DepthToSpace(w_factor=1)(test)
    print(f"SpaceToDepth 和 DepthToSpace 操作后是否相等: {torch.equal(test, im)}")

    # 分解
    tic = time()
    nlevs = 4
    xlo, xhi = ContourDec(nlevs)(im)
    if isinstance(xhi, list):print('xhi is a list')
    print(f"xlo的形状: {xlo.shape}")
    print(f"xhi的数量: {len(xhi)}")
    if len(xhi) > 0:
        print(f"xhi[0]的形状: {xhi[0].shape}")

    # 保存低频子带
    if xlo is not None:
        first_xlo = xlo[0]
        first_xlo = first_xlo.permute(1, 2, 0).cpu().numpy()
        first_xlo = np.clip(first_xlo * norm, 0, 255).astype(np.uint8)
        Image.fromarray(first_xlo).save("xlo.png")

    # 保存高频子带
    if len(xhi) > 0:
        first_xhi = xhi[0][0].permute(1, 2, 0)
        first_xhi_norm = normalize_subband(first_xhi)
        first_xhi_norm = cv2.resize(first_xhi_norm, (480, 320))
        Image.fromarray(first_xhi_norm).save("xhi1_normalized.png")
        
        # 高频子带（对数缩放）
        first_xhi_log = log_scale_subband(xhi[0][0].permute(1, 2, 0))
        first_xhi_log = cv2.resize(first_xhi_log, (480, 320))
        Image.fromarray(first_xhi_log).save("xhi1_log_scaled.png")
        
        # 高频子带（对比度拉伸）
        first_xhi_contrast = contrast_stretching(first_xhi_norm)
        first_xhi_contrast = cv2.resize(first_xhi_contrast, (480, 320))
        Image.fromarray(first_xhi_contrast).save("xhi1_contrast_stretched.png")
        
        # 高频子带（伪彩色）
        first_xhi_colored = apply_colormap(first_xhi_norm)
        first_xhi_colored = cv2.resize(first_xhi_colored, (480, 320))
        Image.fromarray(first_xhi_colored).save("xhi1_colored.png")

    xhi_npy = [x.cpu().numpy() for x in xhi]

    print(f"分解耗时: {time() - tic} 秒")

    # 如果有多个高频子带，进行进一步测试
    if len(xhi) > 4:
        def test_PS(hi_1, h_factor, w_factor):
            hi_1_cat = torch.cat(hi_1, dim=1)
            tmp = DepthToSpace(h_factor=h_factor, w_factor=w_factor)(hi_1_cat)
            print(f"DepthToSpace 输出形状: {tmp.shape}")
            # show_coeff(tmp)  # 确保 show_coeff 正常工作
            tmp = SpaceToDepth(h_factor=h_factor, w_factor=w_factor)(tmp)
            print(f"SpaceToDepth 后是否相等: {torch.equal(hi_1_cat, tmp)}")
    ...
    # 显示子带系数（假设 show_coeff 已定义）
    # show_coeff(xhi)

    # 重构
    tic = time()
    rec = ContourRec()([xlo, xhi])
    print(f"重构图像的形状: {rec.shape}")

    # 保存重构图像
    first_rec = rec[0]
    first_rec = first_rec.permute(1, 2, 0).cpu().numpy()
    first_rec = np.clip(first_rec * norm, 0, 255).astype(np.uint8)
    Image.fromarray(first_rec).save("rec.png")

    print(f"重构耗时: {time() - tic} 秒")
    ...
    nlevs = 2
    contour_dec = ContourDec(nlevs)
    xlo, xhi = contour_dec(input_tensor)
    loss = xlo.mean() + sum([x.mean() for x in xhi])
    loss.backward()

    print(input_tensor.grad is not None)
