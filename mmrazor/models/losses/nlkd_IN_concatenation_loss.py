...
class attentionMatrix(nn.Module):
    def __init__(self, 
        ...
        self.concatination = nn.Sequential(
    ...
    def forward(self, y_s, y_t):
        ...
        theta_y_s = self.theta(y_s).view(batch_size, self.inter_channels, -1, 1)

        phi_y_t = self.phi(y_t).view(batch_size, self.inter_channels, 1, -1)

        h = theta_y_s.size(2)
        w = phi_y_t.size(3)
        theta_y_s = theta_y_s.repeat(1, 1, 1, w)
        phi_y_t = phi_y_t.repeat(1, 1, h, 1)


        concat_feature = torch.cat([theta_y_s, phi_y_t], dim=1)
        f = self.concatination(concat_feature)
        b, _, h, w = f.size()
        f = f.view(b, h, w)
        N = f.size(-1)
        f_div_C = f / N
...
class NLKD_IN_Concatenation_Loss(nn.Module):
    def __init__(self, 
                 in_channels, 
                 inter_channels=None, 
                 dimension=2, 
                 ...
                 tau=1.0, 
                 loss_weight=1.0):
        super(NLKD_IN_Concatenation_Loss, self).__init__()
        self.in_channels = in_channels
...
if __name__ == '__main__':
    ...
    nlkd_loss = NLKD_IN_Concatenation_Loss(in_channels=256, dimension=2)
