...
class attentionMatrix(nn.Module):
    ...
    def forward(self, y_s, y_t):
        ...
        theta_y_s = y_s.view(batch_size, self.in_channels, -1)
        theta_y_s = theta_y_s.permute(0, 2, 1)

        phi_y_t = self.phi(y_t).view(batch_size, self.in_channels, -1)
...
class NLKD_IN_Gaussian_Loss(nn.Module):
    def __init__(self, 
                 in_channels, 
                 inter_channels=None, 
                 dimension=2, 
                 ...
                 tau=1.0, 
                 loss_weight=1.0):
        super(NLKD_IN_Gaussian_Loss, self).__init__()
        self.in_channels = in_channels
...
if __name__ == '__main__':
    ...
    nlkd_loss = NLKD_IN_Gaussian_Loss(in_channels=256, dimension=2)
